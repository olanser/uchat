...
static void create_sql_request(t_file_message message, t_server_users *user,
                               char *buf) {
...
static char *create_response_to_users(t_server *server_info, 
    ...
    sprintf(sql, "select msg_id, msg_chat_id, msg_creator, msg_send_time, "
            "msg_avatar, msg_type, msg_file_name, msg_file_type, msg_file_size "
            "from msg where msg_creator = %d and msg_status = 5 and msg_chat_id"
            " = %d and msg_file_name = '%s' ORDER by msg_id DESC LIMIT 1;",
...
char *mx_end_of_file(t_server *server_info, t_server_users *user) {
    ...
    if (mx_check_file(request, &message, user) == 1) {
        ...
        response = mx_create_response(request[0], *(int *)(&request[1]),
                                     (char)103);
    }
    else {
