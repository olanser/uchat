...
// static int save_file(void *request, t_server_users *user, t_file_message *file) {
//     char path[63];
//     int file_fd;
...
//     file_fd = open(path, O_RDWR | O_CREAT | O_TRUNC, S_IWRITE | S_IREAD | O_APPEND);
...
//     sprintf(buf, "INSERT INTO msg (msg_creator, msg_send_time, msg_file_type, msg_chat_id, msg_file_size, msg_status, msg_file_name) VALUES (%s, datetime('now'), %d, %s, %d, %s, '%s');", user->id_users, message.file_type, message.id_chat, message.size, "5", message.true_name);
...
//     mx_create_response(request, &response, (char)201);
//     if (mx_check_file(((char*)request), &message) == 0) {
//         fprintf(MX_ERROR_THREAD, "not messege\n");
//         response[9] = (char)100;
//         //return 10;
//     }
//     if (mx_check_user_in_char(message.id_chat, user->id_users) == 0) {
//         fprintf(MX_ERROR_THREAD, "permission\n");
//         response[9] = (char)103;
//         //return 10;
//     }
...
//         if (save_file(request, user, &message) == 0) {
//             if (mx_do_query(sql_request,0,0, server_info) == 0) {
//                 response[9] = (char)200;
//                 sprintf(sql_request, "SELECT msg_id from msg where msg_creator = '%s' ORDER by msg_id DESC LIMIT 1;", user->id_users);
