...
char *mx_get_msgs_time(t_server *server_info, t_server_users *user) {
    ...
    if (check_time(&user->buff[9]))
        return mx_create_respons_error_and_log(server_info, user,
            "ERROR TIME FORM!!", MX_QS_ERR_FUNC);
    sprintf(sql, "select msg.msg_id, msg.msg_chat_id, msg.msg_creator, "
        ...
        "g_file_type from msg where msg.msg_chat_id in (select "
        "cou.cou_chat_id from cou where cou.cou_usr_id == %d) and "
        "msg.msg_send_time >= '%s';", user->id_users, &user->buff[9]);
