#include "server.h"
#include "defines.h"

static int callback_one(void *data, int column, char **name, char **tabledata) {
    mx_push_char_users((t_user_in_chat **)data, name[0], name[1]);
    return 0;
}


static int count_response_len(t_user_in_chat *list) {
    int len = 20;

    while (list) {
        len = len + 62;
        list = list->next;
    }
    return len;
}

static void build_res(t_user_in_chat *list, char **response, int len, 
                        void *req) {
    *response = malloc(sizeof(char) * len);
    int pointer = 20;

    memset(*response, 0, sizeof(char)*len);
    (*response)[0] = 7;
    mx_memcpy(&(*response)[1], &((char *)req)[1], 4);
    *(int*)(&(*response)[5]) = len;
    mx_memcpy(&(*response)[9], &req[9], 11);
    while (list) {
        mx_memcpy(&(*response)[pointer], list->usr_id, strlen(list->usr_id));
        pointer = pointer + 11;
        mx_memcpy(&(*response)[pointer], list->usr_nickname, 
                    strlen(list->usr_nickname));
        pointer = pointer + 51;
        list = list->next;
    }
}

static void build_answer(t_server_users *user, t_user_in_chat **list, 
                         t_server *server_info) {
    char sql[200];

    sprintf(sql, "SELECT usr_id, usr_nickname from user where usr_id IN "
    "(SELECT cou_usr_id from cou where cou_chat_id=%s);", &user->buff[9]);
    mx_do_query(sql, callback_one, list, server_info);
}

char *mx_get_chat_users(t_server *server_info, t_server_users *user) {
    t_user_in_chat *list = NULL;
    int res_len = 10;
    char *response = NULL;
    char *request = user->buff;

    if(mx_check_user_in_chat(&(request[9]), user->id_users, server_info)) {
        build_answer(user, &list, server_info);
        res_len = count_response_len(list);
        build_res(list, &response, res_len, request);
        mx_delete_chat_users(&list);
    }
    else {
        response = mx_create_response(request[0], request[1], (char)107);
        fprintf(MX_ERROR_THREAD, "permission\n");
    }
    return response;
}
