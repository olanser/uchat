...
static int set_tuser(char *request, t_table_user *tuser) {
    ...
    while (j < 4) {
        if (end > 1023) {
            fprintf(MX_ERROR_THREAD, "response error\n");
            return 1;
        }
        if (request[++end] == 0) {
            *((&tuser->first_name) + j) = mx_strndup(&request[start], end - start);
            start = end + 1;
            j++;
        }
    }
...
int change_db(t_user *uset, t_table_user *tuser) {
    ...
    sql = mx_strjoin("insert into user values(NULL, '", tuser->first_name);
    sql = mx_strjoin2(sql, "','");
    sql = mx_strjoin2(sql, tuser->second_name);
    sql = mx_strjoin2(sql, "', '");
    sql = mx_strjoin2(sql, tuser->nickname);
    sql = mx_strjoin2(sql, "', '");
    sql = mx_strjoin2(sql, tuser->pass);
    sql = mx_strjoin2(sql, "', 1);");
