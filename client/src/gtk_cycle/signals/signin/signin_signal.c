...
gboolean mx_resize_signin_window (GtkWidget *window, t_objects *objs) {
    ...
    gtk_widget_set_size_request(
        GTK_WIDGET(objs->s_signin_win->signin_main_box), width, height);
    gtk_widget_set_size_request(
        GTK_WIDGET(objs->s_signin_win->signin_child_box), width, height);
    gtk_widget_set_size_request(
        GTK_WIDGET(objs->s_signin_win->image_name), width, -1);
    return FALSE;
}
